...
// (function () {
    ...
    function circles_generator(scheme, completion, objective, launch, schemeID) {
        ...
        for (let index = 0; index < number_of_circles; index++) {
            if (index==0) {
                ...
                radii.push(radii[index-1]-radius/number_of_circles);
            }
         
            
        }
    
        svg = d3.select('#concCircle').append('div').attr('id', schemeID).attr("width", WIDTH).append('svg').attr("width", WIDTH).attr("height", HEIGHT)
            .append('g').attr("transform", `translate(${WIDTH/2},${HEIGHT/2})`)
    
        radius = a + b * THETA;
        completionCircle = Math.round(number_of_circles * completion)
        radii = radii.reverse()
        // svg.append('circle')
        //     .attr("class", 'radius_indicator')
        //     .attr('r', radius);

        // svg.append('circle').datum(radii)
        //     .attr("class", 'radius_indicator')
        //     .attr('r', radius/2);
     svg.selectAll('circle').data(radii)
        .enter()
        .append('circle')
        .attr("class", (d,i)=> (i+1==number_of_circles ? 'radius_indicator' : 'none'))
        .attr('r',d=> {return d})
        .style('fill', (d,i) => { 
            if (schemeID == "scheme0") {
                console.log('schmeid is;'+ schemeID);
                console.log("I value is :"+ i);
                console.log("completionCircle  value is :"+ completionCircle);
                console.log("completion value is :"+ completion);
            }
            return i+1 <= completionCircle ? '#F9E800' :'none'})
        .style('stroke', '#EEEEEE')
...
test = d3.select('#tooltipsGroup')
    ...
    d3.json("/schemes.json", function (data) {
        ...
        data.forEach(function (d, i) {
            circles_generator(d.Scheme, d.Completion, d.Objective, d.Launch, 'scheme' + i);
        });
